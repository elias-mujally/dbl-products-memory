...
52. The 2026-09-02 lab experiment qualifies a frozen single-EFA12026 official backup/restore image for consistent bounded reads. Treat a customer/DBA-provided backup plus independently prepared read-only store as a real offline/fallback candidate, not default production adoption or permission for DBL to administer the customer SQL instance. Reader authority remains separate from backup/restore preparation; FMMA remains unsuitable. V1 database/dependency scope, multi-database consistency, representative performance, security and retention remain gates; Source Schema Inventory is still NOT STARTED as of this qualification.
53. The 2026-09-03 minimum Motakamel V1 semantic scope is customers/products/inventory/sales/returns, with correct identity, eligibility, currency, amounts, quantity/direction and relationships; full payment-channel labels, GL export and UI dictionaries are not prerequisites. Preserve raw payment discriminators where source keys/grouping need them. Multi_Lang is OPTIONAL for this minimum, not globally unnecessary: no indispensable required-code binding to S_FLAGS is proven. EFA12026-alone sufficiency still requires targeted Golden Dataset; do not expand grants/backups or claim connector readiness. Bills.Bill_Type != journal.doc_type dictionary unless independently proven. See `MOTAKAMEL_PLUS_V1_SEMANTIC_SCOPE_2026-09-03.md`; no Canonical Model or Connector change is authorized by this decision.
